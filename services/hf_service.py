    ...
    def __init__(self):
        ...
        if self.vision_provider == 'bedrock':
            try:
                ...
                print(f"[HuggingFaceService] Bedrock client initialized with model: {getattr(Config, 'BEDROCK_VISION_MODEL', 'amazon.nova-lite-v1:0')}")
    ...
    def get_image_caption(self, image_path, model='blip'):
        ...
        if self.vision_provider == 'bedrock' and self.bedrock_client:
            try:
                ...
                print(f"[Bedrock Vision] Failed to generate caption via Bedrock: {bedrock_err}. Falling back to local/HF.")
